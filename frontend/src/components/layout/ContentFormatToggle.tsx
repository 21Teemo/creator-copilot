...
export default function ContentFormatToggle() {
  ...
  return (
    <div className="flex items-center justify-center gap-2 flex-wrap">
      <span className="hidden sm:inline text-xs font-medium text-studio-text-secondary tracking-wider uppercase mr-1">
        Content Format
      </span>
      <div className="relative flex items-center bg-studio-surface p-1 rounded-full border border-studio-border/60">
        {/* Sliding Indicator Background */}
        <div
          className={`absolute h-7 w-[70px] sm:w-[84px] rounded-full transition-transform duration-300 ease-out bg-accent left-[4px] ${
            contentFormat === "short" ? "translate-x-[70px] sm:translate-x-[84px]" : "translate-x-0"
          }`}
        />
        ...
        <button
          ...
          className={`relative z-10 flex items-center justify-center gap-1.5 w-[70px] sm:w-[84px] h-7 text-xs font-semibold rounded-full transition-colors duration-200 cursor-pointer ${
            contentFormat === "long"
