...
import { cancelActiveVideoRender } from "@/lib/intentRouter";
import { Film, AlertCircle, RefreshCw, Layers, Download, X } from "lucide-react";
...
export default function VideoView({ onPush }: VideoViewProps) {
  ...
  const { videoUrl, renderStatus, renderProgress, renderStep, renderElapsedSec, renderError } =
    useMediaStore();
  const { contentFormat, addAudioEnabled } = useProjectStore();
    ...
    return (
      <div className="flex flex-col items-center justify-center flex-1 text-center py-12">
        ...
        <p className="text-xs text-studio-text-secondary max-w-sm">
          Click <strong className="text-studio-text-primary">FFmpeg Render</strong> below. Toggle{" "}
          <strong className="text-studio-text-primary">Add Audio</strong> first if you want
          voiceover narration.
        </p>
      </div>
    );
        ...
        {isRendering ? (
          ...
          >
            ...
            <div className="text-center space-y-1">
              ...
              <p className="text-xs text-studio-text-secondary">
                {renderStep ||
                  (addAudioEnabled
                    ? "Stitching voiceover and scene visuals..."
                    : "Stitching scene visuals (no voiceover)...")}
              </p>
            </div>
            ...
            <div className="flex items-center gap-2 text-[10px] text-studio-text-secondary">
              ...
              <span>
                ...
                {addAudioEnabled ? " · with voiceover" : " · silent"}
              </span>
            </div>
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl border border-studio-border/60 text-xs font-semibold text-studio-text-secondary hover:text-studio-text-primary hover:bg-studio-border/20 cursor-pointer transition-colors"
            >
              <X size={12} />
              Cancel Render
            </button>
          ...
          <div className="text-center max-w-sm p-6 bg-studio-surface border border-studio-border/60 rounded-3xl space-y-4">
            ...
            <p className="text-xs text-studio-text-secondary leading-relaxed">
              {renderError ||
                "The render worker stopped unexpectedly (often out of memory). Use segmented mode (RENDER_SEGMENTED=true) and restart with ./dev.sh start, then try again."}
            </p>
            <button
              type="button"
              onClick={handleRegenerateVideo}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-accent hover:bg-accent/80 text-xs font-semibold text-white mx-auto cursor-pointer"
            >
              <RefreshCw size={12} />
              Re-submit Render
            </button>
            ...
            >
              ...
              <div className="absolute top-3 right-3 opacity-80 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-200 z-20 flex items-center gap-1.5">
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg bg-black/60 border border-white/10 hover:border-accent/40 text-studio-text-secondary hover:text-accent cursor-pointer transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  <RefreshCw size={12} />
                </button>
              </div>
